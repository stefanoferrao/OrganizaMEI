...
document.addEventListener("DOMContentLoaded", function () {
    ...
    form.addEventListener("submit", async function (e) {
      ...
      if (nome && quantidade >= 0) {
        ...
        try {
          ...
          const movimentacaoId = gerarIdentificadorUnico();
          const movimentacao = {
            id: movimentacaoId,
            produto: nome,
            categoria: 'Entrada',
    ...
    saidaModalForm.addEventListener("submit", async function (e) {
      ...
      if (!isNaN(idx) && !isNaN(qtd) && qtd > 0 && produtos[idx].quantidade >= qtd && !isNaN(valor) && valor >= 0) {
        ...
        try {
          ...
          const idCompartilhado = gerarIdentificadorUnico();
  ...
  async function sincronizarMovimentacoesIniciais() {
    ...
    if (estoqueAtivo && typeof sincronizarEstoque === 'function') {
      console.log('Sincronizando movimentações iniciais...');
      ...
      if (movimentacoesSalvas) {
        ...
        console.log('Movimentações carregadas:', movimentacoesEstoque.length);
        recalcularEstoque();
        renderizarProdutos();
      }
    } else {
